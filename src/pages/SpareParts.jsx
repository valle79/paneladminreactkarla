  ...
  return (
    <>
      <div className="page-head">
        ...
        {can('SPARE_PARTS_CREATE') && (
          <button className="btn btn-yellow btn-lg" onClick={openAdd}><Icon name="plus" size={17} /> Agregar Repuesto</button>
        )}
      </div>
                  ...
                  <tr key={r.id}>
                    ...
                    <td>
                      <div className="row-actions">
                        {can('SPARE_PARTS_UPDATE') && <button className="btn-icon" onClick={() => openEdit(r)} title="Editar"><Icon name="edit" size={14} /></button>}
                        {can('SPARE_PARTS_DELETE') && <button className="btn-icon danger" onClick={() => remove(r)} title="Eliminar"><Icon name="trash" size={14} /></button>}
                      </div>
                    </td>
                  </tr>
                );
        ...
        footer={
          <>
            ...
            <button className="btn btn-yellow" onClick={save} disabled={busy}>
              {busy ? <span className="spinner" /> : <Icon name="save" size={15} />} Guardar repuesto
            </button>
          </>
        }
      >
        ...
        <div className="field">
          <label><Icon name="add-list" size={14} /> Especificaciones</label>
          {form.specifications.map((s, i) => (
            <div className="grid-2" style={{ marginBottom: 8 }} key={i}>
              ...
              <div className="flex">
                ...
                <button className="btn-icon danger" onClick={() => setForm((f) => ({ ...f, specifications: f.specifications.filter((_, j) => j !== i) }))}><Icon name="trash" size={14} /></button>
              </div>
            </div>
          ))}
          <button className="btn btn-outline btn-sm" onClick={addSpec}><Icon name="add-list" size={14} /> Añadir especificación</button>
        </div>

        <div className="field">
          <label><Icon name="checkmark--v1" size={14} /> Características</label>
          {form.features.map((f, i) => (
            <div className="flex" style={{ marginBottom: 8 }} key={i}>
              ...
              <button className="btn-icon danger" onClick={() => setForm((s) => ({ ...s, features: s.features.filter((_, j) => j !== i) }))}><Icon name="trash" size={14} /></button>
            </div>
          ))}
          <button className="btn btn-outline btn-sm" onClick={addFeature}><Icon name="plus" size={14} /> Añadir característica</button>
        </div>

        <div className="grid-2">
          <div className="field">
            <label><Icon name="document" size={14} /> Ficha técnica (PDF)</label>
